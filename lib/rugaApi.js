...
const ytmp3 = async (url) => new Promise((resolve, reject) => {
    ...
    .then((res) => {
		...
		if (Number(res.data.filesize.split(' MB')[0]) >= 20.00) resolve({status: 'filesize', link: fileyt, judul: '[❗] Terjadi kesalahan mungkin file audionya terlalu besar'})
...
const ytmp4 = async (url) => new Promise((resolve, reject) => {
    ...
    .then((res) => {
		if (res.data.error) resolve({status: 'error', link: eroryt, judul: res.data.error})
		if (Number(res.data.filesize.split(' MB')[0]) >= 50.00) resolve({status: 'filesize', link: fileyt, judul: '[❗] Terjadi kesalahan mungkin file audionya terlalu besar'})
        resolve({status: 'success', link: res.data.result, size: res.data.filesize, thumb: res.data.thumb, judul: res.data.title})
    })
    .catch((err) =>{
        reject(err)
