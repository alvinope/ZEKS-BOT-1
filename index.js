...
const start = (aruga = new Client()) => {
	...
	await aruga.sendText(chat.id, `Sorry, Sekarang Pay 5k/bulan`).then(() => {
    ...
    aruga.onGlobalParicipantsChanged(async (event) => {
        ...
        if (event.action === 'add' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `ALLO KAK @${event.who.replace('@c.us', '')} \n\nBeliin Seblak Buat Member Yak AWOKAWOK *[ZEKSBOT]*`)
        }
        // kondisi ketika seseorang dikick/keluar dari group
        if (event.action === 'remove' && event.who !== host) {
            await aruga.sendTextWithMentions(event.chat, `TELAH KELUAR @${event.who.replace('@c.us', '')}, Semoga Tenang Amin AWOKAWOK *[ZEKSBOT]*`)
        }
    })

    aruga.onIncomingCall(async (callData) => {
        // ketika seseorang menelpon nomor bot akan mengirim pesan
        await aruga.sendText(callData.peerJid, 'Maaf VC/TELP AUTO BLOK.')
        .then(async () => {
    ...
    aruga.onMessage(async (message) => {
        aruga.getAmountOfLoadedMessages() // menghapus pesan cache jika sudah 3000 pesan.
            .then((msg) => {
                if (msg >= 3000) {
                    console.log('[aruga]', color(`Bused Pesan Gw ${msg}, Hapus Dlu Yekan`, 'yellow'))
                    aruga.cutMsgCache()
                }
            })
