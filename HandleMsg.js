...
module.exports = HandleMsg = async (aruga, message) => {
    try {
        ...
        // Menu and TnC
        case 'speed':
            await aruga.sendText(from, `Nih\nSpeed: ${processTime(t, moment())} _Second_`)
            break
        case 'server':
        case 'ping':
            const os=require('os')
            const loadedMsgg = await aruga.getAmountOfLoadedMessages()
            const chatIds = await aruga.getAllChatIds()
            const groups = await aruga.getAllGroups()
            aruga.reply(from, `Penggunaan RAM: ${(process.memoryUsage().heapUsed / 1024 / 1024).toFixed(2)}MB / ${Math.round(require('os').totalmem / 1024 / 1024)}MB\nCPU: ${os.cpus()[0].model}\n\nStatus :\n- ${loadedMsgg} Loaded Messages\n- ${groups.length} Group Chats\n- ${chatIds.length - groups.length} Personal Chats\n- ${chatIds.length} Total Chats\n\nSpeed: ${processTime(t, moment())} Second`, id)
            break
        case 'tnc':
        ...
        case 'help':
            ...
            break
        case 'qrcode':
	    if (args.length === 2)  return aruga.reply(from, 'Kirim perintah */qrcode [terserah tulisan apa]*\n contoh */qrcode blbot.rf.gd/tutorial* ', id)
                 if(!ark.lenght >= 2) return
                 let qrcodes = body.slice(8)
                 await aruga.sendFileFromUrl(from, `https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=${qrcodes}`, 'gambar.png', '*Processing Sukses ZEKS BOT*').catch(err => console.log('[ERROR] send image'))
        ...
        case 'getpic':
            ...
            aruga.sendFile(from, sesPic, 'session.png', 'kepo anjir lu', id)
            break
        case 'groupinfo' :
            if (!isGroupMsg) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan dalam group!', message.id)
            ...
            await aruga.sendFileFromUrl(from, pfp, 'group.png', `➸ *Name : ${groupname}* 
*➸ Members : ${totalMem}*
*➸ Group Description* 
${desc}`)
            break
        ...
        case 'menuadmin':
            ...
            if (!isGroupAdmins) return aruga.reply(from, 'Admin Lu?', id)
        ...
        case 'ownerbot':
        case 'creator':
            aruga.sendContact(chatId, `6289630152417@c.us`)
            aruga.reply(from, 'Itu Nomor By Aku Tapi Boong :)', id)
            break
        case 'join':
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Euy Gw Bot Kalo Mau Pake Ketik ${prefix}menu`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Maaf Kak Skrng Join Gc 5k`, id)
        ...
        if (!isOwner) return aruga.reply(from, 'Owner Grup Lu?', id)
        if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup!', id)
        ...
        //Owner Bot
        case 'ban':
            if (!isOwnerBot) return aruga.reply(from, 'Owner Lu?', id)
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *Z E K S  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *Z E K S B C* 〙\n\n${msg}`)
            }
